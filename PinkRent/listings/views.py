...
def shop_page(request):
    ...
    # Define hardcoded color choices with translations
    COLOR_CHOICES = {
        'en': {
            'black': 'Black',
            'white': 'White',
    ...
    if parent_category_id and parent_category_id != 'all':
    ...
    # Pagination
    paginator = Paginator(listings, 16)  # 16 listings per page
    ...
    # Create unique color list based on current language
    unique_colors = [(color, COLOR_CHOICES[current_language][color]) for color in COLOR_CHOICES[current_language].keys()]
